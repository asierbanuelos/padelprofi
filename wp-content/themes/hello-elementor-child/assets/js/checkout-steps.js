	...
	class MMCheckout {
		...
		handleStep4Submit() {
			...
			if ( isPayPal ) {
				...
				if ( mid.includes( 'stripe_klarna' ) || mid.includes( 'klarna_payments' ) ) {
					...
					const klarnaGwRadio = document.querySelector( `input[name="payment_method"][value="${ this._step4PaymentMethod }"]` );
		...
		updateReviewSections() {
			...
			if ( reviewPayment ) {
				...
				if ( ! payLabel || /^zahlungsoptionen$/i.test( payLabel ) ) {
					payLabel = /mm_klarna|stripe_klarna|klarna_payments/.test( mid ) ? 'Klarna'
		...
		bindWooCommerceUpdates() {
			...
			$( document.body ).on( 'updated_checkout', () => {
				...
				const savedKlarnaGw = /stripe_klarna|klarna_payments/.test( this._step4PaymentMethod ) ? this._step4PaymentMethod : null;
				...
				if ( this.currentStep === 3 ) {
					...
					if ( savedVirtual ) {
						...
						// Restaurar gateway Klarna separado (stripe_klarna u oficial)
		...
		injectKlarnaOption() {
			...
			// Renombrar el label de Stripe → "Kredit- / Debitkarte"
			const stripeLi = stripeRadio.closest( '.wc_payment_method' );
			const stripeLbl = stripeLi?.querySelector( 'label' );
			if ( stripeLbl ) {
				// TreeWalker recorre TODOS los nodos de texto (incluye span, strong, etc.)
				const walker = document.createTreeWalker( stripeLbl, NodeFilter.SHOW_TEXT );
				let textNode;
				while ( ( textNode = walker.nextNode() ) ) {
					if ( /zahlungsoptionen|zahlungs/i.test( textNode.textContent ) ) {
						textNode.textContent = textNode.textContent.replace(
							/zahlungsoptionen|zahlungs\s*optionen/gi, 'Kredit- / Debitkarte'
						);
						break;
					}
				}
				// Fallback: si el label contiene un elemento hoja con el texto
				if ( /zahlungsoptionen/i.test( stripeLbl.textContent ) ) {
					stripeLbl.querySelectorAll( '*' ).forEach( ( el ) => {
						if ( ! el.children.length && /zahlungsoptionen/i.test( el.textContent ) ) {
							el.textContent = 'Kredit- / Debitkarte';
						}
					} );
				}
			}
		...
		renderStep4Action() {
			...
			if ( isExpress ) {
				...
				// ── Klarna nativo (oficial klarna_payments) o via Stripe UPE (mm_klarna) ──
				...
				if ( mid.includes( 'mm_klarna' ) ) {
					// Stripe UPE: activar la pestaña Klarna en el panel off-screen
					const tryTab = ( n = 0 ) => {
				...
				// Para klarna_payments: selección del radio ya se hizo al elegir — solo mostrar botón
		...
		openPaymentPopup() {
			...
			document.querySelectorAll( '.mm-payment-wrapper .wc_payment_method' ).forEach( ( methodEl ) => {
				...
				if ( ! displayLabel || /^zahlungsoptionen$/i.test( displayLabel ) ) {
					displayLabel = /stripe_klarna|klarna_payments/.test( rv ) ? 'Klarna'
